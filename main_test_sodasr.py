...
def main():
    ...
    test_results['psnr_b'] = []
    test_results['lpips'] = []
    psnr, ssim, psnr_y, ssim_y, psnr_b, lpips_val = 0, 0, 0, 0, 0, 0
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        if img_gt is not None:
            ...
            img1 = lpips.im2tensor(lpips.load_image(str(save_dir)+'/'+str(imgname)+'_SwinIR.png'))
            img0 = img0.to(device_lp)
            img1 = img1.to(device_lp)
            lpips_val = loss_fn_alex.forward(img0,img1).item()
            img0.cpu()
            img1.cpu()
            ...
            print('Testing {:d} {:20s} - PSNR: {:.2f} dB; SSIM: {:.4f}; '
                  ...
                  'PSNR_B: {:.2f} dB; '
                  'LPIPS: {:.3f}.'.
                  format(idx, imgname, psnr, ssim, psnr_y, ssim_y, psnr_b, lpips_val))
        else:
    ...
    if img_gt is not None:
        ...
        print('-- Average LPIPS: {:.3f}'.format(ave_lpips))
...
#     imgname = imgname[0:-1]
#     imgname = imgname + '1'
#     return str(args.folder_lq)+'/'+str(imgname)+str(imgext)
