...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        self.P_lossfn = PerceptualLoss().to(self.device)
        self.RL_lossfn = nn.L1Loss().to(self.device)
        self.RL_lossfn_weight = self.opt_train['RL_lossfn_weight']
    # ----------------------------------------
    # define optimizer
    # ----------------------------------------
    ...
    def netG_forward(self):
        ...
        self.E_LPF = self.netG.module.wt3(self.E.permute(0,2,3,1))[:,0,...]
        self.L_LPF = self.netG.module.wt1(self.L.permute(0,2,3,1))[:,0,...]

    # ----------------------------------------
    # update parameters and get loss
    # ----------------------------------------
    def optimize_parameters(self, current_step,cof):
        self.G_optimizer.zero_grad()
        self.netG_forward()
        G_loss = self.G_lossfn_weight * self.G_lossfn(cof*self.E, cof*self.H) + self.P_lossfn_weight * self.P_lossfn(self.E, self.H) +self.RL_lossfn_weight * self.RL_lossfn(self.E_LPF,self.L_LPF)
        G_loss.backward()
